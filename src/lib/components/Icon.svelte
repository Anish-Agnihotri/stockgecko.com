...
{#if src.length == 1}
	<div class={extraClass}>
		<img src={src[0]} {alt} class="size-5 rounded-full" />
	</div>
{/if}
...
{#if src.length == 2 && nested}
	<div class="{extraClass} relative size-5">
		<img src={src[1]} {alt} class="absolute top-0 left-px z-0 size-4.25 rounded-full" />
		<img src={src[0]} {alt} class="absolute right-0 bottom-0 z-10 size-2.5 rounded-full" />
	</div>
{/if}
