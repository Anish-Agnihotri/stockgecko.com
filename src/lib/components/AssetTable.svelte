...
<div
	class="border-b border-b-gecko-shade bg-gecko-black px-2 py-0.5 text-right font-mono text-xs text-gecko-gray/30 uppercase md:hidden"
>
	<!-- FIXME: it annoys me a little that the arrow is not vertically aligned -->
	<span>Scrollable <span class="text-lg">↔</span> </span>
</div>
...
<Table.Root class="w-full min-w-3xl table-fixed">
	<Table.Header class="bg-gecko-black">
		<Table.Row class="border-b-gecko-shade text-xs font-light">
			...
			<!-- Mid px change -->
			<Table.Head class="w-24 px-0">24h</Table.Head>
			<!-- Volume -->
	...
	<Table.Body>
		{#each snapshot.index.assetsByVolume as { asset, previousIndex }, i}
			...
			>
				...
				<!-- Current ranking -->
				<Table.Cell class="w-8 px-0 text-center align-middle"><Numeric value={i + 1} /></Table.Cell>
